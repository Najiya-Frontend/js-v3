// js/app/hotelinfo.js — Hotel Information page module (ES5 / Tizen 4 safe)
(function (w) {
  "use strict";
  if (w.HotelInfoPage) return;
  ...
  function ensureDom() {
    ...
    viewEl = qs("view-hotelinfo");
    if (!viewEl) return;

    bgEl = viewEl.querySelector(".fc-bg"); // reuse same class names

    pageTitleEl = qs("hi-page-title");
    submenuEl = qs("hi-submenu");
    detailEl = qs("hi-detail");
    detailTitleEl = qs("hi-detail-title");

    sliderViewportEl = qs("hi-slider-viewport");
    sliderTrackEl = qs("hi-slider-track");
    descriptionEl = qs("hi-description");

    leftArrowEl = qs("hi-slider-arrow-left");
    rightArrowEl = qs("hi-slider-arrow-right");
    mainImgEl = qs("hi-main-img");
  }
  ...
  function normalizeItem(x) {
    ...
    if (x.slider && typeof x.slider === "string") {
      try { sliderData = JSON.parse(x.slider); } catch (e) { sliderData = []; }
    ...
    if (!images.length && x.icon) {
      images.push({ src: rewriteAssetUrl(x.icon), duration: 3 });
    }
  ...
  function renderSubmenu() {
    ...
    for (i = 0; i < items.length; i++) {
      item = items[i];
      html += '<div class="fc-submenu-item" data-index="' + i + '">' + escapeHtml(item.name) + "</div>";
    }
  ...
  function moveSubmenu(delta) {
    ...
    var ni = clamp(focusSubmenuIndex + delta, 0, items.length - 1);
    if (ni === focusSubmenuIndex) return;
    focusSubmenuIndex = ni;
    applySubmenuFocus();
    renderDetail();
  }
  ...
  function moveSlider(delta) {
    ...
    var ni = clamp(focusSliderIndex + delta, 0, item.images.length - 1);
    if (ni === focusSliderIndex) return;
  ...
  function handleKeyDown(e) {
    ...
    if (k === RIGHT && focusArea === "submenu") {
      var it = items[focusSubmenuIndex];
      if (it && it.images && it.images.length) {
        focusArea = "slider";
        focusSliderIndex = 0;
        sliderScrollX = 0;
        applySubmenuFocus();
    ...
    if (focusArea === "submenu") {
      ...
      if (k === OK) {
        var it2 = items[focusSubmenuIndex];
        if (it2 && it2.images && it2.images.length) {
          focusArea = "slider";
          focusSliderIndex = 0;
          sliderScrollX = 0;
          applySubmenuFocus();
    ...
    if (k === BACK1 || k === BACK2 || k === BACK3 || k === BACK4) return false;
    return false;
  }
